...
class TestBuildNeuronPaths:
    ...
    def test_build_neuron_paths_invalid_indices(self):
        ...
        connections = [
            (0, 1),
            (5, 10),
        ]  # Second connection has invalid indices
...
class TestAddPointsDataset:
    ...
    def test_add_points_dataset(self, mock_viewer, mock_atlas):
        ...
        data = np.array(
            [
                [
                    ...
                    1000.0,
                    1000.0,
                ],  # Should become [40, 40, 40] in pixels
                [
                    2000.0,
                    2000.0,
                    2000.0,
                ],  # Should become [80, 80, 80] in pixels
            ]
        )

        with (
            patch("brainrender_napari.utils.visualize_datasets.show_info"),
        ...
        np.testing.assert_allclose(
            points_added[0], [40.0, 40.0, 40.0], rtol=0.1
        )
    ...
    def test_add_points_dataset_empty(self, mock_viewer, mock_atlas):
        ...
        with patch(
            "brainrender_napari.utils.visualize_datasets.show_error"
        ) as mock_error:
            _add_points_dataset(
                data, mock_viewer, "test_dataset", mock_atlas, None
            )
    ...
    def test_add_points_dataset_out_of_bounds(self, mock_viewer, mock_atlas):
        ...
        with patch(
            "brainrender_napari.utils.visualize_datasets.show_info"
        ) as mock_info:
            _add_points_dataset(
                data, mock_viewer, "test_dataset", mock_atlas, None
            )
