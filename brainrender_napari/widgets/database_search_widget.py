...
class DatabaseSearchWidget(QWidget):
    ...
    def __init__(self, parent: QWidget = None):
        ...
        self.database_combo.addItems(
            ["Allen Brain Atlas", "Janelia MouseLight", "NeuroMorpho.org"]
        )
    ...
    def _on_search_clicked(self):
        ...
        try:
            ...
            if "Allen" in db_name:
                ...
                regions = [
                    r.strip()
                    for r in self.mouselight_region_input.text().split(",")
                    if r.strip()
                ]
                ...
                species = (
                    self.neuromorpho_species_input.text().strip() or "mouse"
                )
                cell_type = (
                    self.neuromorpho_celltype_input.text().strip() or None
                )
                brain_region = (
                    self.neuromorpho_region_input.text().strip() or None
                )
                worker = self._search_neuromorpho_worker(
                    searcher, species, cell_type, brain_region
                )
            else:
    ...
    def _search_allen_worker(self, searcher, structure: Optional[str]):
        ...
        return searcher.search_allen_neurons(
            structure_area=structure, limit=50
        )

    @thread_worker
    def _search_mouselight_worker(self, searcher, regions: Optional[list]):
        """Worker thread for MouseLight database search."""
        return searcher.search_mouselight_neurons(
            filter_regions=regions, limit=50
        )

    @thread_worker
    def _search_neuromorpho_worker(
        self,
        searcher,
        species: str,
