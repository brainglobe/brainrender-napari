...
def _resize_to_match(data: np.ndarray, target_shape: tuple) -> np.ndarray:
    ...
    for dim in range(min(len(data.shape), len(target_shape))):
        if data.shape[dim] > target_shape[dim]:
            ...
            data = np.take(
                data, range(start, start + target_shape[dim]), axis=dim
            )
    ...
    # Copy to result
    slices = tuple(
        slice(0, min(data.shape[i], target_shape[i]))
...
def _ensure_proper_layer_visibility(viewer: Viewer, atlas_name: str):
    ...
    dataset_layers = [
        ...
        for l in viewer.layers
        if (
            "_streamlines" in l.name
            or "_points" in l.name
            or "_volume" in l.name
        )
...
def _add_connected_points(
    ...
    if connections:
        try:
            ...
            line_segments = []
            for p_idx, c_idx in connections[
                :100
...
def add_dataset_to_viewer(
    dataset_id: str, viewer: Viewer, atlas_name: str = None
):
    """
    ...
    try:
        ...
        if data_type == "points":
            _add_points_dataset(
                data, viewer, dataset_id, bg_atlas, existing_atlas_annotation
            )
            ...
            _add_volume_dataset(
                ...
                bg_atlas,
                metadata,
                existing_atlas_annotation,
            )
        elif data_type == "streamlines":
            _add_streamlines_dataset(
                data, viewer, dataset_id, bg_atlas, existing_atlas_annotation
            )
        else:
            show_error(
...
def _add_points_dataset(
    ...
    if len(data.shape) != 2 or data.shape[1] != 3:
        show_error(f"Expected points data in shape (N, 3), got {data.shape}")
        return
    ...
    resolution = (
        atlas.resolution
    )  # e.g., [25.0, 25.0, 25.0] microns per pixel for allen_mouse_25um
    ...
    if len(resolution) >= 3:
        ...
        if len(atlas_shape) >= 3:
            valid_mask = (
                (points_pixel[:, 0] >= 0)
                & (points_pixel[:, 0] < atlas_shape[0])  # Z bounds
                & (points_pixel[:, 1] >= 0)
                & (points_pixel[:, 1] < atlas_shape[1])  # Y bounds
                & (points_pixel[:, 2] >= 0)
                & (points_pixel[:, 2] < atlas_shape[2])  # X bounds
            )
            valid_count = np.sum(valid_mask)
            if valid_count < len(points_pixel):
                show_info(
                    f"Placed {valid_count}/{len(points_pixel)} points within atlas bounds. "
                    f"Atlas shape: {atlas_shape}, Resolution: {resolution} μm/pixel"
                )
                points_pixel = points_pixel[valid_mask]
            else:
                show_info(
                    f"All {len(points_pixel)} points are within atlas bounds. "
                    f"Atlas shape: {atlas_shape}, Resolution: {resolution} μm/pixel"
                )
    else:
    ...
    if hasattr(points_layer, "out_of_slice_display"):
        points_layer.out_of_slice_display = (
            True  # Show points outside current slice in 3D
        )
    ...
    try:
        if viewer.dims.ndisplay == 3:
            ...
            if points_layer in viewer.layers:
                viewer.layers.move(
                    viewer.layers.index(points_layer), len(viewer.layers) - 1
                )
    except Exception:
        pass
        ...
        if actual_format == "nrrd":
            ...
            try:
                ...
                error_messages.append(
                    "pynrrd not installed - install with: pip install pynrrd"
                )
            except Exception as e:
        ...
        if volume_data is None:
            ...
            try:
                ...
                try:
                    ...
                    error_messages.append(
                        f"imageio workaround error: {str(e)}"
                    )
            except Exception as e:
                ...
                try:
                    ...
                    error_messages.append(
                        f"Final imageio attempt error: {str(e2)}"
                    )
            else:
        ...
        if volume_data is not None:
            ...
            if dataset_shape != atlas_shape:
                ...
                try:
                    ...
                    zoom_factors = [
                        atlas_shape[i] / dataset_shape[i]
                        for i in range(
                            min(len(atlas_shape), len(dataset_shape))
                        )
                    ]
                    ...
                    volume_data = ndimage.zoom(
                        volume_data, zoom_factors, order=1, mode="constant"
                    )
                    # Crop or pad to exact match if needed
                    if volume_data.shape != atlas_shape:
                        volume_data = _resize_to_match(
                            volume_data, atlas_shape
                        )
                except ImportError:
                    ...
                    try:
                        ...
                        if (
                            len(volume_data.shape) == 3
                            and volume_data.shape[0] > 1
                        ):
                            resized_slices = []
                            for z in range(
                                min(volume_data.shape[0], atlas_shape[0])
                            ):
                                img = Image.fromarray(volume_data[z])
                                img_resized = img.resize(
                                    (atlas_shape[2], atlas_shape[1]),
                            ...
                            if volume_data.shape[0] < atlas_shape[0]:
                                ...
                                volume_data = np.concatenate(
                                    [volume_data]
                                    + [last_slice]
                                    * (atlas_shape[0] - volume_data.shape[0]),
                                    axis=0,
                                )
                            ...
                            volume_data = np.array(img_resized)[
                                np.newaxis, :, :
                            ]
                            # Replicate to match z dimension
                            if volume_data.shape[0] < atlas_shape[0]:
                                volume_data = np.repeat(
                                    volume_data, atlas_shape[0], axis=0
                                )
                    except Exception as e:
                        # If resizing fails, show warning but continue
                        show_info(
                            f"Note: Dataset dimensions ({dataset_shape}) don't match atlas ({atlas_shape}). "
            ...
            is_2d_projection = (
                len(volume_data.shape) == 3 and volume_data.shape[0] == 1
            )
            ...
            if atlas_annotation_layers:
                annotation_idx = viewer.layers.index(
                    atlas_annotation_layers[0]
                )
            ...
            if is_2d_projection:
                ...
                show_info(
                    ...
                    "Use the dimension slider (horizontal arrow at bottom) to navigate slices. "
                    "Hover over the atlas annotation layer to see brain structure names. "
                    "Tip: Adjust dataset opacity (0.2-0.3) in layer controls if layers overlap."
                )

            return
        else:
            detected_info = (
                f" (detected as {actual_format})"
                if actual_format != file_format
                else ""
            )
            error_msg = (
                f"Failed to load volume file: {data.name}{detected_info}\n\n"
            )
            if error_messages:
                error_msg += (
                    "Errors encountered:\n"
...
def _add_streamlines_dataset(
    ...
    if isinstance(data, dict) and "points" in data and "connections" in data:
        ...
        resolution = (
            atlas.resolution
        )  # e.g., [25.0, 25.0, 25.0] microns per pixel

        if (
            len(resolution) >= 3
            and len(points_microns.shape) == 2
            and points_microns.shape[1] == 3
        ):
            # Convert from microns to pixels
            ...
            if len(atlas_shape) >= 3:
                ...
                if valid_count == 0:
                    show_error(
                        f"No streamline points are within atlas bounds. Atlas shape: {atlas_shape}, Resolution: {resolution} μm/pixel"
                    )
                    return
                elif valid_count < len(points_pixel):
                    show_info(
                        f"Placed {valid_count}/{len(points_pixel)} streamline points within atlas bounds."
                    )
                    ...
                    show_info(
                        f"All {len(points_pixel)} streamline points are within atlas bounds. Neuron properly fitted in atlas."
                    )
            else:
                show_info(
                    "Using streamline data (atlas shape validation skipped)"
                )
        else:
            show_error(
                f"Unexpected data format. Expected points array with shape (N, 3), got {points_microns.shape}"
            )
            return
        ...
        if connections and len(connections) > 0:
            ...
            scale = [1.0, 1.0, 1.0]  # Default pixel space scale
            atlas_layers = [
                l for l in viewer.layers if atlas.atlas_name in l.name
            ]
            if atlas_layers:
                try:
                    scale = atlas_layers[0].scale  # Match atlas layer scale
            ...
            if paths:
                try:
                    ...
                    show_info(
                        f"Using points visualization (shapes error: {str(e)})"
                    )
        ...
        if hasattr(streamlines_layer, "out_of_slice_display"):
            streamlines_layer.out_of_slice_display = (
                True  # Show paths outside current slice
            )
        ...
        try:
            if streamlines_layer in viewer.layers:
                viewer.layers.move(
                    viewer.layers.index(streamlines_layer),
                    len(viewer.layers) - 1,
                )
        except Exception:
            pass
        ...
        if len(data.shape) == 2 and data.shape[1] == 3:
            ...
            if len(resolution) >= 3:
                ...
                if len(atlas_shape) >= 3:
                    ...
                    if valid_count < len(points_pixel):
                        show_info(
                            f"Placed {valid_count}/{len(data)} streamline points within atlas bounds. "
                            f"Atlas shape: {atlas_shape}, Resolution: {resolution} μm/pixel"
                        )
                        points_pixel = points_pixel[valid_mask]
                    else:
                        show_info(
                            f"All {len(points_pixel)} streamline points are within atlas bounds."
                        )
            else:
                # Fallback: assume data is already in correct format
                points_pixel = data
                show_info(
                    "Using streamline data as-is (atlas resolution format unexpected)"
                )
            ...
            show_error(f"Unsupported streamline data format: {data.shape}")
    else:
